...
for i, scenario in enumerate(complex_scenarios, 1):
    ...
    sql_base = sql_base.replace('<think>\n', '').replace('<think>', '').strip()
    sql_250 = sql_250.replace('<think>\n', '').replace('<think>', '').strip()
    sql_500 = sql_500.replace('<think>\n', '').replace('<think>', '').strip()
...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    for i, r in enumerate(results, 1):
        ...
        f.write("CHECKPOINT-250:\n")
        f.write(r['sql_250'] + "\n\n")
        
        f.write("CHECKPOINT-500:\n")
        f.write(r['sql_500'] + "\n\n")
        
        f.write("ANALISE QUALITATIVA (para ser preenchida):\n")
        f.write("  Corretude (BASE):     [  ]/10\n")
        f.write("  Corretude (250):      [  ]/10\n")
        f.write("  Corretude (500):      [  ]/10\n")
        f.write("  Otimizacao (BASE):    [  ]/10\n")
        f.write("  Otimizacao (250):     [  ]/10\n")
        f.write("  Otimizacao (500):     [  ]/10\n")
        f.write("  Completude (BASE):    [  ]/10\n")
        f.write("  Completude (250):     [  ]/10\n")
        f.write("  Completude (500):     [  ]/10\n")
...
print("="*100)
print(f"{'#':<3} {'CENARIO':<35} {'BASE-OK?':<10} {'250-OK?':<10} {'500-OK?':<10} {'MELHOR':<10}")
print("-"*100)
for i, r in enumerate(results, 1):
    # Check if SQL starts with SELECT (basic validation)
    base_ok = "SIM" if r['sql_base'].upper().startswith('SELECT') or 'SELECT' in r['sql_base'].upper()[:50] else "NAO"
    ckp250_ok = "SIM" if r['sql_250'].upper().startswith('SELECT') or 'SELECT' in r['sql_250'].upper()[:50] else "NAO"
    ckp500_ok = "SIM" if r['sql_500'].upper().startswith('SELECT') or 'SELECT' in r['sql_500'].upper()[:50] else "NAO"
    
    print(f"{i:<3} {r['name']:<35} {base_ok:<10} {ckp250_ok:<10} {ckp500_ok:<10} {'[ANALISAR]':<10}")
